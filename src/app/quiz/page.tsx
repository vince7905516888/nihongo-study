              ...
              {(["vocabulary", "grammar"] as QuizType[]).map((t) => (
                <button
                  ...
                  className={`flex-1 py-2 rounded-lg font-medium transition-colors ${
                    quizType === t ? "bg-purple-500 text-white" : "bg-gray-100 text-gray-600 hover:bg-gray-200"
                  }`}
                >
                  {t === "vocabulary" ? "📖 單字" : "📝 文法"}
                </button>
              ...
              {["N5", "N4", "N3", "N2", "N1"].map((l) => (
                <button
                  ...
                  className={`px-4 py-2 rounded-lg font-medium transition-colors ${
                    level === l ? "bg-purple-500 text-white" : "bg-gray-100 text-gray-600 hover:bg-gray-200"
                  }`}
                >
                  {l}
                </button>
        ...
        <div className="bg-white border border-gray-200 rounded-2xl p-6 space-y-5">
          ...
          {feedback && (
            <div className={`rounded-lg p-3 text-sm ${feedback.correct ? "bg-green-50 text-green-700" : "bg-red-50 text-red-700"}`}>
              {feedback.correct ? "✅ 正確！" : `❌ 錯誤，答案是：${q.answer}`}
